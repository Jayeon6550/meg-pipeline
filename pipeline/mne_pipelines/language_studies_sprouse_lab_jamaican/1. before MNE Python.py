...
for i,j in names:
    ...
    fiducials = pd.read_csv(stylus_file, sep=r'\s+', skiprows=3, header=None) #the delimiter means whitespace
...
for i,j in names:
    ...
    list_path = os.path.join(DATASET_PATH, i, "stimuli")
    save_path = os.path.join(DATASET_PATH, i, "meg")
    list_file = j+'.csv'
