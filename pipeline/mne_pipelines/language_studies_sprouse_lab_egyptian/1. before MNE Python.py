...
#Install Pycharm
#location of interpreter in Jon's installation
#/Applications/MNE-Python/1.10.2_0/.mne-python/bin/python

#--------------
#Option 1: Install Spyder
#--------------
...
#May want to change the run selection shortcut to command-enter (it is f9 by default)
...
#To run MEG160 on a mac, use Codeweaver (company name) Crossover.
#-create new bottle
#-open C drive
#-copy Meg160 to C drive (this is the only place it can go)
#-go into Meg160->Bin->offlineMeg160.exe
...
for i,j in names:
    head_path = "/Users/jrs9906/Desktop/egyptian data/"+i+"/meg/"
    stylus_file = j+'.txt'
...
I am going to change the names of the files BY HAND to match the subject number.
"""
...
for i in range(n):  #loop through and append each name to the list
    folder_name = "sub-"+f'{i+1:03}'#for the path
    file_name = 'egyptian_list'+f'{i+1:02}' #the list
    names.append((folder_name, file_name))


#Now we can loop through the names tuple, and access each of these names
for i,j in names:
    list_path = "/Users/jrs9906/Desktop/egyptian data/"+i+"/stimuli/"
    save_path = "/Users/jrs9906/Desktop/egyptian data/"+i+"/meg/"
    list_file = j+'.csv'
...
#Takes 5 files: The data, the 2 marker files, headshape is called acq-head_headshape in our files, and fiducials is called acq-points_headshape_edited in our files
#Saves as .fif
