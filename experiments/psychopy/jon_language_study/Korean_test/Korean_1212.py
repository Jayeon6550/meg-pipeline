...
for trialIndex in range(totalTrials):
    ...
    # ============================================================
    #  (A) ### FIX: BREAK LOGIC (practice/real 분리 + block별 남은 문장 수)
    #  - break row는 CSV에서 block=0이므로, real break는 last_task_block 기준으로 계산
    # ============================================================
    if sentence_text == breakKeyword:
        ...
        if is_practice_end_break:
            ...
            # real break는 last_task_block(마지막 real block)을 사용
            break_block = last_task_block if last_task_block in (1, 2, 3) else 1

            done = block_real_done[break_block]
            total = block_real_total[break_block]
