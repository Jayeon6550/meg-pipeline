...
trialList = data.importConditions('test2.csv')
...
stimuliSize = 2
instructionSize = 1
wordOn =  42 #36 #300ms #42 #350ms
wordOff = 24 #24 #200ms #24 #200ms
lastWordOn = 132  #120  #1000 #132  #1100
...
stim = visual.TextStim(win, text= 'Inna dah experiment yah, yah go read di sentences\n' 
                                  'one word at a time while we record di MEG.\n\n'
                                  'Afta some a di sentence dem, yah go get asked\n'
                                  'one Yes or No question bout di sentence.\n\n'
                                  'Di question dem very simple.\n'
                                  'A just fi mek sure yah read di sentence dem.\n\n'
                                  'All yah fi do a read di sentence dem normally,\n'
                                  'and den answer di question.\n\n'
                                  'Please nuh badda blink during di sentence.\n'
...
for trialIndex in range(startItem - 1, totalTrials):
    ...
    if trialList[trialIndex]['sentence'] == breakKeyword:
        ...
        if currentBreakCount == 1:
            stim = visual.TextStim(win, text= 'Congratulations! Yuh answer %i a di %i practice question dem correct.\n\n '
...
event.clearEvents()
stim = visual.TextStim(win,
                       text='Congratulations, yuh finish now!\n\n'
                            'Yuh read %i sentence,\n'
                            'and answer %i outta %i question correct!\n\n'
                            'Thanks fi yuh participation.' % (
